...
// ------- helpers UI (refonte style) -------
const cx = (...c: Array<string | false | null | undefined>) => c.filter(Boolean).join(" ");

const THEME = {
    // Palette & tokens
    brandGrad: "bg-gradient-to-r from-teal-600 to-indigo-600",
    brandGradHover: "hover:from-teal-500 hover:to-indigo-500",
    brandRing: "focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-400/50",
    ...
    shadowHover: "hover:shadow-xl hover:shadow-slate-900/10 transition-shadow",
    pill: "bg-white/70 border border-teal-200 text-slate-800",

    // Typo
    head: "text-3xl md:text-5xl font-extrabold tracking-tight",
    sub: "text-base md:text-lg text-slate-600",
};

export type ButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "solid" | "outline" | "ghost" };
    ...
    ({ className, variant = "solid", ...props }, ref) => (
        <button
            ...
            className={cx(
                ...
                variant === "outline" && `bg-white/70 backdrop-blur ${THEME.border} text-slate-900 hover:bg-white ${THEME.brandRing}`,
                variant === "ghost" && `bg-transparent text-slate-800 hover:bg-white/50 ${THEME.brandRing}`,
                variant === "solid" && `${THEME.brandGrad} ${THEME.brandGradHover} text-white ${THEME.brandRing} shadow-md`,
                className
            )}
            {...props}
        />
...
// Styles utilitaires (tokens)
const ink = THEME.textMain;
const mute = THEME.textSub;
const head = THEME.head;
const sub = THEME.sub;
const paper = cx(THEME.glass, THEME.border, THEME.shadow);
    ...
    const slides: Slide[] = [
        {
            ...
            title: (
                <div className="flex items-center gap-3">
                    <span className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-white font-bold ring-1 ring-white/40 shadow-sm bg-gradient-to-br from-teal-600 to-indigo-700">
                        GC
                    </span>
                    <span>TRAVEL GC</span>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    >
                        {[
                            ...
                            <motion.li
                                key={i}
                                className={`px-3 py-1.5 rounded-full text-sm ${THEME.pill}`}
        ...
        },
        {
            ...
            accent: <ColorBlock caption="Schéma rapide du parcours" color="bg-teal-100 text-teal-900" />,
        },
        {
            key: "transport",
    ...
    return (
        <div className="relative min-h-[80vh] w-full font-sans bg-gradient-to-b from-slate-50 via-teal-50 to-indigo-50">
            <div className="relative max-w-6xl mx-auto p-4 md:p-8">
                {/* Header */}
                <div className={`mb-3 rounded-xl p-3 md:p-4 flex items-center justify-between bg-white/60 backdrop-blur ${THEME.border}`}>
                    <div className="flex items-center gap-3">
                        <span className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-white font-bold ring-1 ring-white/40 shadow-sm bg-gradient-to-br from-teal-600 to-indigo-700">
                            GC
                        </span>
                ...
                {!showInscription && (
                    <div className="mb-6 h-1.5 rounded-full bg-slate-200/80 overflow-hidden">
                        ...
                        >
                            {/* animation sûre (pas de classe arbitraire) */}
                        ...
                        <div className="grid md:grid-cols-[1fr,240px] gap-6">
                            ...
                            <div className="hidden md:block">
                                <motion.div
                                    ...
                                    className={`rounded-xl h-full p-4 text-center text-sm bg-white/50 backdrop-blur ${THEME.border}`}
                                >
                                    {slides[index].accent}
                                </motion.div>
                            </div>
    ...
    return (
        <div className="relative">
            ...
            <div className="grid md:grid-cols-[1fr,240px] gap-6">
                ...
                <div className="hidden md:block">
                    <div className={`rounded-xl h-full p-4 text-center text-sm ${THEME.border} ${THEME.glass}`}>
...
function ColorBlock({ caption, color = "bg-teal-100 text-teal-800" }: { caption: string; color?: string }) {
        ...
        <div className={`rounded-xl p-5 ${paper}`}>
            ...
            <div className="mt-4 grid sm:grid-cols-3 gap-3 text-sm">
                {[
                    ...
                    <div key={k} className={`rounded-lg ${THEME.border} ${THEME.glass} p-3`}>
...
function KawaiiRail() {
    ...
    const pt = (pathRef.current && lenRef.current > 0)
        ...
        : ({ x: 30, y: 150 } as unknown as DOMPoint);
        ...
        <div className="space-y-5">
            <div className="grid md:grid-cols-3 gap-4">
                {cities.map((c, i) => (
                    <motion.button
                        ...
                        onClick={() => setSel(i)}
                        whileHover={{ y: -2 }}
                        className={`text-left rounded-xl ${THEME.border} ${THEME.glass} p-4 ${i === sel ? "ring-2 ring-teal-500" : "hover:border-teal-300"}`}
                    >
                        <div className="flex items-center justify-between">
            ...
            <AnimatePresence mode="wait">
                <motion.div
                    ...
                    className={`rounded-xl ${THEME.border} ${THEME.glass} p-5`}
                >
...
function HighlightsGrid() {
    const items = [
        { title: "Bains sous les étoiles", desc: "Soirée aux thermes, ambiance unique (safe & chill)", color: "from-rose-400 to-rose-500", icon: <Bath className="w-5 h-5" /> },
        ...
        { title: "Vieux quartiers & cafés", desc: "Ruelles de Bratislava, cafés cosy", color: "from-teal-400 to-teal-500", icon: <Map className="w-5 h-5" /> },
        { title: "Museums & chill à Vienne", desc: "Culture la journée, parc en fin d’aprèm", color: "from-indigo-400 to-indigo-500", icon: <Train className="w-5 h-5" /> },
    ];
    return (
        <StaggerGrid cols="md:grid-cols-2">
            {items.map((it, i) => (
                <motion.div key={i} whileHover={{ y: -2 }} className={`rounded-xl ${THEME.border} ${THEME.glass} overflow-hidden`}>
...
function InscriptionForm({ onSent }: { onSent?: () => void }) {
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // IMPORT DYNAMIQUE : évite tout crash au chargement initial
            const { default: emailjs } = await import("@emailjs/browser");

            await emailjs.send(
                "service_TravelGCtest",
                "template_alrwijz",
                {
                    prenom: prenom.trim(),
                    nom: nom.trim(),
        ...
        <form onSubmit={submit} className="space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
                <label className="text-sm">
                    ...
                    <input
                        className="w-full px-4 py-2 rounded-lg bg-white border border-slate-300 focus:outline-none focus:border-teal-500"
                        value={prenom}
            ...
            <label className="text-sm">
                ...
                <input
                    type="email"
                    className={`w-full px-4 py-2 rounded-lg bg-white border ${email && !validEmail(email) ? "border-red-400" : "border-slate-300"} focus:outline-none focus:border-teal-500`}
                    value={email}
            ...
            <label className="text-sm">
                ...
                <input
                    inputMode="numeric"
                    className={`w-full px-4 py-2 rounded-lg bg-white border ${sciper && !validSciper(sciper) ? "border-red-400" : "border-slate-300"} focus:outline-none focus:border-teal-500`}
                    value={sciper}
