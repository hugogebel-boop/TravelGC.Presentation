...
/* ---------- Slide Itinéraire — V2 clean (sans bloc superflu, sans CTA) ---------- */

function ItinerarySlideV2() {
    ...
    const cities: City[] = [
        {
            ...
            sections: [
                { title: "Incontournables", points: ["Parlement & Chaînes", "Bains Széchenyi / Gellért", "Citadelle, vues nocturnes"] },
                { title: "Génie civil", points: ["Ponts du Danube (structures)", "Métro M1 (patrimoine technique)"] },
                { title: "Moments chill", points: ["Ruin bars (en groupe)", "Bains en soirée (option)"] },
            ],
        },
        {
            key: "bratislava",
            ...
            sections: [
                { title: "Incontournables", points: ["Château & vieille ville", "Rives du Danube", "Street-art & cafés"] },
                { title: "Génie civil", points: ["Pont SNP (haubané)", "Aménagements fluviaux"] },
                { title: "Moments chill", points: ["Cafés étudiants", "Golden hour sur le Danube"] },
            ],
        },
        {
            key: "vienna",
            ...
            sections: [
                { title: "Incontournables", points: ["Ring & musées", "Prater / parcs", "Cafés viennois"] },
                { title: "Génie civil", points: ["Donau City (urbanisme)", "U-Bahn & hubs multimodaux"] },
                { title: "Moments chill", points: ["Parcs fin d’aprèm", "Option concert"] },
            ],
        },
    ];

    const days: { day: string; city: CityKey; what: string }[] = [
        { day: "J1", city: "budapest", what: "Arrivée (nuit). Bains & centre." },
        { day: "J2", city: "budapest", what: "Ponts + visites. Soirée conviviale." },
        { day: "J3", city: "bratislava", what: "Train matin. Vieille ville & SNP." },
        { day: "J4", city: "vienna", what: "Train court. Musées & parcs." },
        { day: "J5", city: "vienna", what: "Visite GC / libre. Café viennois." },
        { day: "J6", city: "vienna", what: "Retour (nuit) → Zurich → Lausanne." },
    ];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-3 gap-3">
                {cities.map((c, i) => (
                    <button
                        ...
                        className={cx(
                            "group rounded-xl bg-white/70 backdrop-blur border border-slate-200/70 p-4 text-left transition-all",
                    ...
                    >
                        ...
                        <div className="mt-2 text-xs text-slate-600">Clique pour voir l’essentiel</div>
                    </button>
                ))}
            </div>

            {/* Panneau contenu — 3 cartes compactes */}
            <AnimatePresence mode="wait">
                <motion.div
                    key={active.key}
                ...
                >
                    {active.sections.map((sec, idx) => (
                        <div
                            key={idx}
                            className="rounded-2xl bg-white/70 backdrop-blur border border-slate-200/70 shadow-lg shadow-slate-900/5 overflow-hidden"
                        >
                            <div className={`h-1.5 w-full bg-gradient-to-r ${active.grad}`} />
                            <div className="p-5">
                                <div className="font-semibold text-slate-900 mb-2">{sec.title}</div>
                                <ul className="text-sm text-slate-700 space-y-1 list-disc pl-5">
                                    {sec.points.map((p, k) => <li key={k}>{p}</li>)}
                                </ul>
                            </div>
                        </div>
                    ))}
            ...
            {/* Timeline jour par jour — compacte, cliquable */}
            <div className="rounded-xl bg-white/70 backdrop-blur border border-slate-200/70 p-4">
                <div className="mb-2 text-slate-800 font-semibold">Timeline (J1 → J6)</div>
                <div className="grid sm:grid-cols-6 gap-2">
                    {days.map((d, i) => {
                        const on = d.city === active.key;
                        const label = d.city === "budapest" ? "Budapest" : d.city === "bratislava" ? "Bratislava" : "Vienne";
                        const badge = d.city === "budapest" ? "🇭🇺" : d.city === "bratislava" ? "🇸🇰" : "🇦🇹";
                        return (
                            <button
                                key={i}
                                onClick={() => {
                                    const idx = cities.findIndex(c => c.key === d.city);
                                    if (idx >= 0) setSel(idx as 0 | 1 | 2);
                                }}
                                className={cx(
                                    "text-left rounded-lg p-3 transition-all bg-white/70 backdrop-blur border border-slate-200/70",
                                    on ? "ring-2 ring-teal-500" : "hover:border-teal-300"
                                )}
                            >
                                <div className="text-xs text-slate-500">{d.day}</div>
                                <div className="text-sm font-semibold text-slate-800 flex items-center gap-1">
                                    <span>{label}</span><span>{badge}</span>
                                </div>
                                <div className="text-xs text-slate-600 mt-1">{d.what}</div>
                            </button>
                        );
                    })}
                </div>
...
/* ---------- Itinéraire & moments forts (existants) ---------- */
