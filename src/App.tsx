...
/* =====================================================
   FIX: Le build cassait car l'import "./ui" n'existait pas.
   → On rend le fichier AUTONOME en définissant ici Button/Card/CardContent.
   → Aucun changement visuel majeur; tu peux remplacer ces composants par ta lib plus tard.

   BONUS: helpers testables + mini tests Vitest (s'exécutent seulement si `describe/it/expect` existent).
   ===================================================== */
...
export const __examples = [
  { case: "progress start", got: progressPercent(0, 6), expect: 100 / 6 },
  { case: "progress end", got: progressPercent(5, 6), expect: 100 },
  { case: "clamp <0", got: clampIndex(-3, 5), expect: 0 },
  { case: "clamp >last", got: clampIndex(42, 5), expect: 5 },
];
if ((globalThis as any).describe && (globalThis as any).it && (globalThis as any).expect) {
  const { describe, it, expect } = (globalThis as any);
  describe("TravelGC helpers", () => {
    it("progressPercent clamps and scales", () => {
      expect(progressPercent(0, 6)).toBeCloseTo(100 / 6, 6);
      expect(progressPercent(5, 6)).toBeCloseTo(100, 6);
      expect(progressPercent(-10, 6)).toBeCloseTo(100 / 6, 6);
      expect(progressPercent(0, 0)).toBe(0);
    });
    it("clampIndex keeps range", () => {
      ...
      body: (
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Button className="rounded-xl px-6 text-base">Je m’inscris</Button>
